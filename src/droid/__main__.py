...
def init_argparse() -> argparse.ArgumentParser:
    ...
    parser.add_argument("-p", "--platform", help="Platform target", choices=['splunk', 'azure', 'Microsoft365DefenderBackend', 'esql', 'eql'])
...
def is_raw_rule(args, base_config):
    ...
    if (
        ...
        (args.platform == 'Microsoft365DefenderBackend' and args.sentinel_mde)) and
        ...
        args.platform in ['splunk', 'azure'] or
        (args.platform == 'Microsoft365DefenderBackend' and args.sentinel_mde)
    ):
        return False
...
def droid_platform_config(args, config_path):
    ...
    if args.platform == 'azure' or args.platform == 'Microsoft365DefenderBackend':
        try:
            with open(config_path) as file_obj:
                content = file_obj.read()
                config_data = tomllib.loads(content)
                if args.platform == 'Microsoft365DefenderBackend' and args.sentinel_mde:
                    # With Azure Sentinel as backend, loads config from azure but keep MDE pipelines
                    config = config_data["platforms"]["azure"]
                    config["pipelines"] = config_data["platforms"]["Microsoft365DefenderBackend"]["pipelines"]
                else:
        ...
        if args.export or args.search:
            ...
            if  (
                    args.platform == 'Microsoft365DefenderBackend' and
...
def main(argv=None) -> None:
    ...
    if args.search:
        ...
        elif args.platform == 'Microsoft365DefenderBackend' and args.sentinel_mde:
            if is_raw_rule(args, base_config):
        ...
        elif args.platform == "Microsoft365DefenderBackend" and not args.sentinel_mde:
