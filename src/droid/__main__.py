...
def init_argparse() -> argparse.ArgumentParser:
    ...
    parser.add_argument("-p", "--platform", help="Platform target", choices=['splunk', 'azure', 'kusto', 'esql', 'eql'])
...
def is_raw_rule(args, base_config):
    ...
    if (
        ...
        return False
    elif args.platform == 'kusto' and raw_rule_folder_name in args.rules:
        return True
    elif (
...
def droid_platform_config(args, config_path):
    ...
    if args.platform == 'azure' or args.platform == 'kusto':
        try:
            with open(config_path) as file_obj:
                content = file_obj.read()
                config_data = tomllib.loads(content)
                if args.platform == 'kusto' and args.sentinel_mde:
                    # With Azure Sentinel as backend, loads config from azure but keep MDE pipelines
                    config = config_data["platforms"]["azure"]
                    config["pipelines"] = config_data["platforms"]["kusto"]["pipelines"]
                else:
...
def main(argv=None) -> None:
    ...
    if args.search:
        ...
        elif args.platform == 'kusto' and args.sentinel_mde:
            if is_raw_rule(args, base_config):
